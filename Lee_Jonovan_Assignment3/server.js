...
app.get("/invoice.html", function (request, response, next) { //created to generate invoice page
...
app.post("/display_cart", function (request, response) { // posts data from the display_cart form, with action named "display_cart" (activated when submit purchase button is clicked in cart)
  if (typeof session.username != "undefined") { // if the session username is not undefined
    response.redirect('./invoice.html'); // they must already be logged in if their username is defined so redirect them to the invoice
...
app.post("/process_form", function (request, response) { // post data from the form sent to proces_purchase
  ...
  if (typeof POST["product_type"] != "undefined") {
    ...
    for (i = 0; i < products[product_type].length; i++) {//FOR loop generates length of product +1. i=i+1 post increment use the value of i firstthen increment
  ...
  console.log(isValidData, hasPurchases);
...
app.post("/register", function (request, response) {
  ...
  username = request.body.username;// Add toLower for the username later
  email = request.body.email;
  password = request.body.password;//case sensitive password
  repeat_password = request.body.repeat_password; //Makes password case sensitive
  errs = [];
